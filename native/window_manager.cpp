...
Napi::Value FocusWindow(const Napi::CallbackInfo& info) {
    ...
    if (info.Length() < 1) {
        Napi::TypeError::New(env, "Expected 1 argument: windowTitle")
            .ThrowAsJavaScriptException();
        return env.Null();
    }

    if (!info[0].IsString()) {
        Napi::TypeError::New(env, "Invalid argument type: windowTitle must be a string")
            .ThrowAsJavaScriptException();
        return env.Null();
    }
    ...
    try {
        ...
        if (hwnd == nullptr) {
            return Napi::String::New(env, "❌ Window '" + windowTitle + "' not found. Make sure the application is running and the window title is correct.");
        }

        // Verify the window is valid
        if (!IsWindow(hwnd)) {
            return Napi::String::New(env, "❌ Invalid window handle for '" + windowTitle + "'.");
        }
        ...
        if (GetWindowPlacement(hwnd, &wp)) {
            if (wp.showCmd == SW_SHOWMINIMIZED || wp.showCmd == SW_HIDE) {
                std::cout << "Window is minimized/hidden, restoring first..." << std::endl;
                ShowWindow(hwnd, SW_RESTORE);
                // Wait a bit for the window to restore
                Sleep(100);
            }
        ...
        if (!success) {
            // Try alternative approach if SetForegroundWindow fails
            // First, try to show the window
            ShowWindow(hwnd, SW_SHOW);
            // Then try to bring it to top
            BringWindowToTop(hwnd);
            // Finally try SetForegroundWindow again
            success = SetForegroundWindow(hwnd);
        }

        if (success) {
            return Napi::String::New(env, "✅ Successfully brought window to foreground: " + windowTitle);
        } else {
            return Napi::String::New(env, "⚠️ Window found but may not have been brought to foreground: " + windowTitle + ". This can happen due to Windows focus restrictions.");
        }
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
    ...
    exports.Set(Napi::String::New(env, "focusWindow"),
                Napi::Function::New(env, FocusWindow));
    return exports;
}
