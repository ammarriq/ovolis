...
import { initCaptureExclusionListener, setAppWindowsExcludedFromCapture } from "~/utils/capture-exclusion";
...
export function registerGlobalIpc() {
    ...
    ipcMain.handle("set-current-window-position", (event, x, y) => {
        const win = BrowserWindow.fromWebContents(event.sender);
    ...
    // Display metrics
    ipcMain.handle("get-display-metrics", (_evt, displayId) => {
        const displays = screen.getAllDisplays();
        const display = displays.find((d) => d.id.toString() === displayId) ?? screen.getPrimaryDisplay();
    ...
    ipcMain.handle("set-exclude-app-windows-from-capture", (_evt, enabled) => {
