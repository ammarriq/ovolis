...
export function registerCameraIpc() {
    ipcMain.handle("open-camera", (_evt, cameraId) => {
        if (!cameraWindow) {
    ...
    ipcMain.handle("get-camera-source-id", async () => {
        try {
            ...
            const match = sources.find((s) => (s.name || "").toLowerCase() === cameraTitle.toLowerCase());
    ...
    ipcMain.on("camera:update-metrics", (_evt, metrics) => {
